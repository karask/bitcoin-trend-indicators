...
Stock history comes from Yahoo Finance's unofficial chart endpoint and is stored as validated completed-session snapshots in Cloudflare D1. The stock page needs no provider account, API key, browser token, or per-symbol setup. A private IndexedDB copy remains only as a rendering accelerator; clearing it does not remove the shared D1 history.
