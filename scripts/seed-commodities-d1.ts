...
import { COMMODITIES, type CommodityDefinition } from "../lib/commodities.ts";
...
const selected = requested ? COMMODITIES.filter(stock => stock.symbol === requested) : COMMODITIES;
if (!selected.length) throw new Error(`Unsupported commodity symbol ${requested}`);
...
async function seed(stock: CommodityDefinition): Promise<string> {
  process.stdout.write(`Preparing ${stock.symbol} from Yahoo Finance... `);
  const result = await fetchCommodityHistory(stock.symbol);
  ...
  const metadata = JSON.stringify({ adjustment: result.adjustment, contractLabel: result.contractLabel, terms: "personal-research" });
  ...
  const file = path.join(output, `commodity-${stock.id}-yahoo.sql`);
...
for (const stock of selected) {
  if (files.length) await new Promise(resolve => setTimeout(resolve, 1_500));
  files.push(await seed(stock));
}
if (apply) {
...
console.log(`${files.length} commodity seed file(s) ${apply ? `applied to ${remote ? "remote" : "local"} D1` : "written"}.`);
