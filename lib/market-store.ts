...
import type { AssetId, MarketDataset, SourceId } from "./market-data.ts";
import { ASSETS, marketDefinition } from "./market-data.ts";
...
export async function readStoredDataset(asset: AssetId, source: SourceId, timeframe: Timeframe): Promise<MarketDataset | null> {
  const database = getDatabase();
  const metadata = database.prepare("SELECT market,retrieved_at,checksum,warning,first_candle,last_candle,candle_count FROM provider_snapshots WHERE asset=? AND source=? AND timeframe=?").get(asset, source, timeframe) as StoredSnapshot | undefined;
  if (!metadata || metadata.last_candle == null || metadata.last_candle < completedBoundary(timeframe)) return null;
  const rows = database.prepare("SELECT time,open,high,low,close,volume,complete FROM market_candles WHERE asset=? AND source=? AND timeframe=? ORDER BY time").all(asset, source, timeframe) as StoredCandle[];
...
export async function persistDataset(dataset: MarketDataset): Promise<void> {
  ...
  const database = getDatabase();
  const upsertCandle = database.prepare("INSERT INTO market_candles (asset,source,timeframe,time,market,open,high,low,close,volume,complete) VALUES (?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(asset,source,timeframe,time) DO UPDATE SET market=excluded.market,open=excluded.open,high=excluded.high,low=excluded.low,close=excluded.close,volume=excluded.volume,complete=excluded.complete");
  const upsertSnapshot = database.prepare("INSERT INTO provider_snapshots (asset,source,timeframe,market,retrieved_at,checksum,warning,first_candle,last_candle,candle_count) VALUES (?,?,?,?,?,?,?,?,?,?) ON CONFLICT(asset,source,timeframe) DO UPDATE SET market=excluded.market,retrieved_at=excluded.retrieved_at,checksum=excluded.checksum,warning=excluded.warning,first_candle=excluded.first_candle,last_candle=excluded.last_candle,candle_count=excluded.candle_count");
  runTransaction(database, () => {
    database.prepare("DELETE FROM market_candles WHERE asset=? AND source=? AND timeframe=?").run(dataset.asset, dataset.source, dataset.timeframe);
    for (const candle of dataset.candles) upsertCandle.run(dataset.asset, dataset.source, dataset.timeframe, candle.time, dataset.market, candle.open, candle.high, candle.low, candle.close, candle.volume, candle.complete ? 1 : 0);
    upsertSnapshot.run(dataset.asset, dataset.source, dataset.timeframe, dataset.market, dataset.retrievedAt, dataset.checksum, dataset.warning, dataset.candles[0]?.time ?? null, dataset.candles.at(-1)?.time ?? null, dataset.candles.length);
  });
}

export async function persistSignalSnapshots(asset: AssetId, source: SourceId, timeframe: Timeframe, candleClose: number | null, snapshots: SignalSnapshot[]): Promise<void> {
