import { COMMODITIES, aggregateCommodityWeeks, type CommodityHistoryResponse } from "./commodities.ts";
import { ASSETS, marketDefinition, type SourceId } from "./markets.ts";
import { STOCKS, aggregateStockWeeks, type StockHistoryResponse } from "./stocks.ts";
...
export const OVERVIEW_ASSETS = [
  ...
  ...COMMODITIES.map(commodity => ({ lab: "commodity" as const, asset: commodity.id, source: "yahoo" as const, symbol: commodity.symbol, label: commodity.label, venue: "Yahoo Finance · COMEX futures · USD/oz" })),
];
export type OverviewAsset = typeof OVERVIEW_ASSETS[number];
export type OverviewHistory = { lab: "crypto"; history: CryptoHistory } | { lab: "stock"; history: StockHistoryResponse } | { lab: "commodity"; history: CommodityHistoryResponse };
...
export const overviewUrl = (asset: OverviewAsset, indicator: string) => viewUrl(asset.lab, { asset: asset.asset, source: asset.source as SourceId | "yahoo", indicator, timeframe: overviewTimeframe(indicator) });
...
export function summarizeOverview(data: OverviewHistory, indicator: string) {
  ...
  const weekly = data.lab === "crypto" ? data.history.weekly.candles : data.lab === "stock" ? aggregateStockWeeks(daily, Date.parse(data.history.retrievedAt)) : aggregateCommodityWeeks(daily, Date.parse(data.history.retrievedAt));
  const options = { market: data.lab === "crypto" ? "crypto" as const : data.lab === "stock" ? "equity" as const : "commodity" as const, asset: data.lab === "crypto" ? data.history.daily.asset : undefined, stock: data.lab === "stock" ? data.history.stock.id : undefined, commodity: data.lab === "commodity" ? data.history.commodity.id : undefined, indicatorIds: [indicator] };
