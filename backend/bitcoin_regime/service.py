...
from .providers import PROVIDERS, aggregate_weekly, kraken
...
class ResearchService:
    ...
    def refresh_source(self, source: str) -> dict:
        try:
            daily = PROVIDERS[source]()
            self._persist(daily)
            weekly = Dataset.create(daily.source, daily.market, "1w", aggregate_weekly(daily.candles), b"aggregate:" + daily.checksum.encode())
            ...
            self.repository.health(source, "healthy", message, daily.candles[-1].time, weekly.candles[-1].time)
            return {"source": source, "status": "healthy", "daily": len(daily.candles), "weekly": len(weekly.candles)}
        except Exception as error:
            self.repository.health(source, "failed", str(error))
            return {"source": source, "status": "failed", "error": str(error)}

    async def refresh_all(self) -> list[dict]:
        async with self._refresh_lock:
            results = []
            for source in PROVIDERS:
                results.append(await asyncio.to_thread(self.refresh_source, source))
            for timeframe in ("1d", "1w"):
                await asyncio.to_thread(self._cross_venue_report, timeframe)
            return results

    def _cross_venue_report(self, timeframe: str) -> None:
        datasets = self.repository.rows("SELECT source,market,first_candle,last_candle FROM datasets WHERE timeframe=? ORDER BY source", [timeframe])
        ...
        for metadata in datasets:
            rows = self.repository.rows("SELECT time,open,high,low,close,volume,complete FROM candles WHERE source=? AND timeframe=? AND time BETWEEN ? AND ? ORDER BY time", [metadata["source"], timeframe, common_start, common_end])
            ...
            dataset = Dataset.create(metadata["source"], metadata["market"], timeframe, candles, b"equal-date")
        ...
        self.repository.save_report(f"cross-venue:{timeframe}", {"timeframe": timeframe, "equalDate": {"start": common_start, "end": common_end}, "costBps": 15, "rankingMethod": "Median cross-venue Calmar", "ranking": ranking, "paretoSet": pareto, "universalWinnerDeclared": False})
