...
export default function StockDashboard() {
  ...
  return <main className="app-shell stock-shell">
    ...
    <TimeframeHint indicator={indicator} market="stock" />
      ...
      {loading && !history ? <LoadingView /> : calculation && history && <>
        <section className={`hero-grid ${loading ? "is-refreshing" : ""}`}><article className="chart-card"><div className="chart-heading"><div><p className="eyebrow">LAST CONFIRMED {timeframe === "1d" ? "DAILY" : "WEEKLY"} ADJUSTED CLOSE · {history.stock.exchange} · YAHOO FINANCE</p><div className="price-line"><strong>{formatPrice(current?.close)}</strong><span className={change != null && change < 0 ? "negative" : ""}>{formatPct(change, true)}</span></div></div><StateBadge state={calculation.selected.readiness?.ready === false ? null : calculation.selected.state} label={calculation.selected.readiness?.ready === false ? "Not ready" : roleStateLabel(calculation.selected.role, calculation.selected.id, calculation.selected.state)} /></div><div className="chart-frame"><ChartExplorer key={`${stockId}:${timeframe}`} candles={calculation.candles} dailyCandles={history.daily} weeklyCandles={history.weekly} selected={calculation.selected} denomination="USD" timeframe={timeframe} theme={theme} />{loading && <div className="chart-refresh">Refreshing stored candles…</div>}</div><div className="chart-legend">{calculation.selected.overlays.filter(line => line.showInLegend !== false).map(line => <span key={line.name}><i style={{ background: chartColorCss(line.color) }} />{line.legendLabel ?? line.name}</span>)}{calculation.selected.ribbons.filter(ribbon => ribbon.showInLegend !== false).flatMap(ribbon => (Object.entries(ribbon.palette) as Array<[RegimeState, string]>).map(([state, color]) => <span key={`${ribbon.id}-${state}`}><i className="range-swatch" style={{ background: chartColorCss(color) }} />{titleState(state)} range</span>))}<span><i className="flip-dot" />Confirmed flip</span><span className="method-note">Signals effective next session open</span></div></article>
          <aside className="signal-panel"><p className="eyebrow">CURRENT EVIDENCE</p><h2>{calculation.selected.shortName}</h2><div className="current-state-row"><StateBadge state={calculation.selected.readiness?.ready === false ? null : calculation.selected.state} label={calculation.selected.readiness?.ready === false ? "Not ready" : roleStateLabel(calculation.selected.role, calculation.selected.id, calculation.selected.state)} /><span>{calculation.selected.readiness?.ready === false ? "Insufficient history" : "Completed candles only"}</span></div><SignalReadiness readiness={calculation.selected.readiness} lastFlip={calculation.selected.lastFlip} timeframe={timeframe} market="equity" />{calculation.selected.readiness?.ready !== false && calculation.selected.role !== "valuation" && (calculation.selected.bullTrigger != null || calculation.selected.bearTrigger != null) && <>{triggerCard(triggerLabels[0], calculation.selected.bullTrigger, "bull-trigger")}{triggerCard(triggerLabels[1], calculation.selected.bearTrigger, "bear-trigger")}</>}<DailyConfirmation signal={calculation.selected} /><div className="method-card"><span>RULE</span><p>{calculation.selected.explanation}</p><b>{calculation.selected.triggerLabel}</b></div>{calculation.selected.id === "kk_supertrend" && <div className="proxy-note"><strong>{calibrationStatus(undefined, timeframe, stockId)}</strong>ATR {calculation.selected.values.atrLength} with factor {calculation.selected.values.factor}. Weekly TSLA, NVDA, GOOGL, MU, SNDK and MSTR use the September stock-reference preset. Bitmine weekly uses an approximate Yahoo-feed shared 15/2 preset (approximately 4% below the reference screenshot flip) with limited history. Daily uses separate approximate preset families with five consecutive confirmations in either direction. SpaceX weekly remains uncalibrated.</div>}{calculation.selected.id === "mayer" && <div className="proxy-note"><strong>Equity interpretation</strong>Shown as the price-to-200-day-average ratio, not as an intrinsic valuation measure.</div>}<p className="disclaimer">Research view only. No live orders or individualized allocation advice.</p></aside></section>
