...
import { historyIsCurrent, loadCryptoHistory, loadStockHistory, requestJson, type SyncStatus } from "../../lib/history-client";
...
export default function AssetOverview() {
    ...
    const check = async () => {
      ...
      const worker = async () => {
        while (!abort.signal.aborted && next < OVERVIEW_ASSETS.length) {
          ...
          const quoteUrl = asset.lab === "crypto" ? `/api/v1/spot?${new URLSearchParams({ asset: asset.asset, source: asset.source })}` : `/api/v1/stocks/quote?symbol=${stockDefinition(asset.asset).symbol}`;
          await Promise.all([
  ...
  return <main className="app-shell overview-shell">
    ...
    <p className="overview-note">Crypto uses each asset’s default venue, shown below. Stock prices use Yahoo Finance; history is split-adjusted. Provisional levels may move before the next close. Conditional models have no guaranteed single-price reversal.</p>
    {(["crypto", "stock"] as const).map(lab => <section className="overview-section" aria-labelledby={`overview-${lab}`} key={lab}>
      <div className="section-heading"><h2 id={`overview-${lab}`}>{lab === "crypto" ? "Crypto" : "Stocks"}</h2><span>{OVERVIEW_ASSETS.filter(asset => asset.lab === lab).length} assets · {spec.shortName}</span></div>
      <table className="overview-table"><thead><tr><th scope="col">Asset / source</th><th scope="col">Daily</th><th scope="col">Weekly</th><th scope="col">{timeframe === "1w" ? "Weekly" : "Daily"} level / condition</th><th scope="col">Price</th><th scope="col">Snapshot / last flip</th></tr></thead><tbody>{OVERVIEW_ASSETS.filter(asset => asset.lab === lab).map(asset => {
        ...
        const flip = ready && signal.lastFlip != null ? signalTiming(signal.lastFlip, timeframe, lab === "crypto" ? "crypto" : "equity") : null;
