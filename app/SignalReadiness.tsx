...
export default function SignalReadiness({ readiness, lastFlip, timeframe, market }: { readiness: SignalSnapshot["readiness"]; lastFlip: number | null; timeframe: Timeframe; market: MarketContext }) {
  ...
  return <dl className="signal-timing"><div><dt>Last reversal confirmed</dt><dd>{timing ? format(timing.confirmedAt) : "No reversal in available history"}</dd></div>{timing && <div><dt>Effective next open</dt><dd>{format(timing.effectiveAt)}</dd></div>}</dl>;
}
