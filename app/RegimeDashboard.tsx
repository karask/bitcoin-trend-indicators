...
export default function RegimeDashboard() {
  ...
  return <main className="app-shell">
    <header className="topbar"><div className="brand-lockup"><div className="brand-mark">{activeAsset.symbol}</div><div><p className="eyebrow">CRYPTO REGIME LAB · {activeAsset.label.toUpperCase()}</p><h1>Trend regimes, without the black box.</h1></div></div><div className="header-actions"><nav className="lab-nav" aria-label="Research labs"><a href="/" aria-current="page">Crypto</a><a href="/stocks">Stocks</a></nav>{installPrompt && <button className="install-button" onClick={() => { (installPrompt as Event & { prompt: () => void }).prompt(); setInstallPrompt(null); }}>Install app</button>}<div className={`freshness ${data && !isCurrent ? "stale" : ""}`}><span />{data ? `${!isCurrent ? "Snapshot behind" : "Confirmed through"} · ${formatDate(confirmedThrough)}` : "Loading exchange data"}</div><AccountControls /><button className="theme-toggle" type="button" onClick={toggleTheme} aria-label={`Switch to ${theme === "dark" ? "light" : "dark"} theme`} title={`Switch to ${theme === "dark" ? "light" : "dark"} theme`}><span aria-hidden="true">{theme === "dark" ? "☀" : "☾"}</span><b>{theme === "dark" ? "Light" : "Dark"}</b></button></div></header>
    ...
    <Watchlist lab="crypto" active={{ asset, source }} revision={data?.dataset.retrievedAt} now={clock} activeQuote={activeSpot} onSelect={pin => { setView(current => ({ ...current, asset: pin.asset, source: pin.source, timeframe: "1w", indicator: "kk_supertrend" })); setRefreshKey(value => value + 1); }} />
    {loading && !data ? <LoadingView /> : data && <>
      ...
      {data.selected.id === "kk_supertrend" && <CalibrationPanel asset={asset} timeframe={timeframe} values={data.selected.values} />}
