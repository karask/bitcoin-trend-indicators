...
import { INDICATOR_SPECS, indicatorDisplayName, familyRows as getFamilyRows, SUPER_GUPPY_R12_DEFAULTS, type SuperGuppyConfig, type SuperGuppySource, type IndicatorGuidance as Guidance } from "../lib/regimes";
...
export default function RegimeDashboard() {
  ...
  const triggerLabels = data?.selected.role === "exit" ? ["SHORT EXIT ABOVE", "LONG EXIT BELOW"] : data?.selected.role === "confirmation" && data.selected.id !== "kk_50_200_ema" ? ["POSITIVE ABOVE", "NEGATIVE BELOW"] : data?.selected.confirmation ? ["BULLISH CONFIRMATION ABOVE", "BEARISH CONFIRMATION BELOW"] : ["BULLISH ABOVE", "BEARISH BELOW"];
  ...
  return <main className="app-shell">
    ...
    {loading && !data ? <LoadingView /> : data && <>
      <section className={`hero-grid ${loading ? "is-refreshing" : ""}`}><article className="chart-card"><div className="chart-heading"><div><p className="eyebrow">LAST CONFIRMED {timeframe === "1d" ? "DAILY" : "WEEKLY"} CLOSE · {data.dataset.market} · {data.dataset.sourceLabel.toUpperCase()}</p><div className="price-line"><strong>{formatPrice(current?.close, data.dataset.denomination)}</strong><span className={change != null && change < 0 ? "negative" : ""}>{formatPct(change, true)}</span></div></div><StateBadge state={data.selected.state} label={roleStateLabel(data.selected.role, data.selected.id, data.selected.state)} /></div><div className="chart-frame"><ChartExplorer key={`${asset}:${source}:${timeframe}`} candles={data.candles} dailyCandles={marketHistory?.daily.candles} weeklyCandles={marketHistory?.weekly.candles} selected={data.selected} denomination={data.dataset.denomination} timeframe={timeframe} theme={theme} />{loading && <div className="chart-refresh">Refreshing completed candles…</div>}</div><div className="chart-legend">{data.selected.overlays.filter(line => line.showInLegend !== false).map(line => <span key={line.name}><i style={{ background: chartColorCss(line.color) }} />{line.legendLabel ?? line.name}</span>)}{data.selected.ribbons.filter(ribbon => ribbon.showInLegend !== false).flatMap(ribbon => (Object.entries(ribbon.palette) as Array<[State, string]>).map(([state, color]) => <span key={`${ribbon.id}-${state}`}><i className="range-swatch" style={{ background: chartColorCss(color) }} />{titleState(state)} range</span>))}{data.selected.id === "super_guppy" ? <><span><i style={{ background: "linear-gradient(90deg,var(--chart-guppy-lime) 0 50%,var(--chart-guppy-red) 50%)" }} />Swing arrows</span><span><i style={{ background: "linear-gradient(90deg,var(--chart-guppy-aqua) 0 50%,var(--chart-guppy-blue) 50%)" }} />Trend Break arrows</span></> : <span><i className="flip-dot" />Confirmed flip</span>}<span className="method-note">States effective next open</span></div></article>
        <aside className="signal-panel"><p className="eyebrow">CURRENT EVIDENCE</p><h2>{data.selected.shortName}</h2><div className="current-state-row"><StateBadge state={data.selected.state} label={roleStateLabel(data.selected.role, data.selected.id, data.selected.state)} /><span>{data.selected.readiness?.ready === false ? "Insufficient history" : "Completed candles only"}</span></div><SignalReadiness readiness={data.selected.readiness} lastFlip={data.selected.lastFlip} timeframe={timeframe} market="crypto" />{data.selected.readiness?.ready !== false && data.selected.role !== "valuation" && (data.selected.bullTrigger != null || data.selected.bearTrigger != null) && <>{triggerCard(triggerLabels[0], data.selected.bullTrigger, "bull-trigger")}{triggerCard(triggerLabels[1], data.selected.bearTrigger, "bear-trigger")}</>}<DailyConfirmation signal={data.selected} /><div className="method-card"><span>RULE</span><p>{data.selected.explanation}</p><b>{data.selected.triggerLabel}</b></div>{data.selected.disclaimer && <div className="proxy-note"><strong>Implementation note</strong>{data.selected.disclaimer}</div>}<p className="disclaimer">Research view only. No live orders or individualized allocation advice.</p></aside></section>
