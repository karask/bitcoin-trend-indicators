...
type SpotQuote = { source: string; sourceLabel: string; market: string; denomination: string; price: number; retrievedAt: string };
type MatrixItem = {
...
type Payload = {
  ...
  dataset: { source: string; sourceLabel: string; market: string; timeframe: Timeframe; retrievedAt: string; checksum: string; stale: boolean; demo: boolean; storage: "provider" | "sqlite" | "demo"; warning: string | null; quality: { gaps: number; duplicates: number; malformed: number }; firstCandle: number | null; lastCandle: number | null; candleCount: number; chartCandleCount: number };
  candles: Candle[];
...
const formatPrice = (value: number | null | undefined) => value == null || !Number.isFinite(value) ? "—" : new Intl.NumberFormat("en-US", { style: "currency", currency: "USD", maximumFractionDigits: 0 }).format(value);
...
function RegimeChart({ candles, selected }: { candles: Candle[]; selected: Payload["selected"] }) {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      for (let i = 0; i <= 4; i++) { const yy = pad.t + i * ch / 4; ctx.beginPath(); ctx.moveTo(pad.l, yy); ctx.lineTo(w - pad.r, yy); ctx.stroke(); ctx.fillText(formatPrice(max - i * (max - min) / 4), w - pad.r + 8, yy + 3); }
      ...
      const drawTrigger = (value: number | null, color: string, label: string) => { if (value == null) return; const yy = y(value); ctx.strokeStyle = color; ctx.lineWidth = 1; ctx.setLineDash([6, 5]); ctx.beginPath(); ctx.moveTo(pad.l, yy); ctx.lineTo(w - pad.r, yy); ctx.stroke(); ctx.setLineDash([]); ctx.fillStyle = "#fffefa"; ctx.fillRect(Math.max(pad.l, w - pad.r - 156), yy - 10, 156, 18); ctx.fillStyle = color; ctx.font = "700 9px ui-monospace, monospace"; ctx.textAlign = "right"; ctx.fillText(`${label} · ${formatPrice(value)}`, w - pad.r - 4, yy + 3); };
...
export default function RegimeDashboard() {
  const [source, setSource] = useState("bitstamp"), [timeframe, setTimeframe] = useState<Timeframe>("1w"), [indicator, setIndicator] = useState("support_band"), [role, setRole] = useState<Role>("regime");
  ...
  const [clock, setClock] = useState(() => Date.now()), [refreshTick, setRefreshTick] = useState(0);
  ...
  useEffect(() => {
    ...
    fetch(`/api/v1/dashboard?source=${source}&timeframe=${timeframe}&indicator=${indicator}`, { signal: controller.signal, cache: "no-store" }).then(async response => {
  ...
  }, [source, timeframe, indicator, refreshTick]);
  useEffect(() => {
    const controller = new AbortController();
    fetch(`/api/v1/spot?source=${source}&request=${Date.now()}`, { signal: controller.signal, cache: "no-store" }).then(async response => {
  ...
  const activeSpot = spot?.source === source ? spot : null;
  ...
  const triggerCard = (label: string, value: number | null, variant: string) => <div className={`trigger ${variant}`}><span>{label}</span><strong>{value == null ? "Conditional" : formatPrice(value)}</strong><small>{data?.selected.thresholdKind} · as of {formatDate(data?.dataset.lastCandle, true)} UTC</small></div>;

  return <main className="app-shell">
    <header className="topbar"><div className="brand-lockup"><div className="brand-mark">₿</div><div><p className="eyebrow">BTC REGIME LAB</p><h1>Trend regimes, without the black box.</h1></div></div><div className="header-actions">{installPrompt && <button className="install-button" onClick={() => { (installPrompt as Event & { prompt: () => void }).prompt(); setInstallPrompt(null); }}>Install app</button>}<div className={`freshness ${data?.dataset.stale ? "stale" : ""}`}><span />{data ? `${data.dataset.stale ? "Stale" : "Completed candles"} · ${formatDate(data.dataset.lastCandle)}` : "Loading exchange data"}</div></div></header>
    ...
    <section className="command-row" aria-label="Research controls"><div className="control-group"><label htmlFor="market">Market source</label><select id="market" value={source} onChange={event => { beginRefresh(); setSource(event.target.value); }}>{(data?.sources ?? [{ id: "bitstamp", label: "Bitstamp", market: "BTC/USD" }]).map(item => <option key={item.id} value={item.id}>{item.label} · {item.market}</option>)}</select></div><div className="control-group grow"><label htmlFor="indicator">Indicator</label><select id="indicator" value={indicator} onChange={event => { beginRefresh(); setIndicator(event.target.value); const selectedRole = data?.registry.find(item => item.id === event.target.value)?.role; if (selectedRole) setRole(selectedRole); }}>{options.map(item => <option key={item.id} value={item.id}>{item.displayName}</option>)}</select></div><div className="segmented" aria-label="Timeframe"><button className={timeframe === "1d" ? "active" : ""} onClick={() => chooseTimeframe("1d")}>1D</button><button className={timeframe === "1w" ? "active" : ""} onClick={() => chooseTimeframe("1w")}>1W</button></div></section>
    <section className="close-countdown" aria-live="polite" aria-label={`${closeClock.title}: ${closeClock.remaining} remaining`}><div><p className="eyebrow">CONFIRMATION CLOCK · UTC</p><strong>{closeClock.title}</strong><span>{closeClock.boundary}</span></div><div className="countdown-value"><b>{closeClock.remaining}</b><small>remaining</small></div><div className="spot-price"><span>LIVE SPOT · {(activeSpot?.sourceLabel ?? data?.dataset.sourceLabel ?? source).toUpperCase()}</span><b>{activeSpot ? formatPrice(activeSpot.price) : "—"}</b><small>{activeSpot ? `fetched ${new Intl.DateTimeFormat("en-GB", { hour: "2-digit", minute: "2-digit", second: "2-digit", hour12: false, timeZone: "UTC" }).format(new Date(activeSpot.retrievedAt))} UTC` : spotError ? "quote unavailable" : "fetching current price…"}</small></div><p>The live quote is informational and refreshes on page load. The open {timeframe === "1d" ? "day" : "week"} is provisional; confirmed indicators update only after its close.</p></section>
    {loading && !data ? <LoadingView /> : data && <>
      <section className={`hero-grid ${loading ? "is-refreshing" : ""}`}><article className="chart-card"><div className="chart-heading"><div><p className="eyebrow">LAST CONFIRMED {timeframe === "1d" ? "DAILY" : "WEEKLY"} CLOSE · {data.dataset.market} · {data.dataset.sourceLabel.toUpperCase()}</p><div className="price-line"><strong>{formatPrice(current?.close)}</strong><span className={change != null && change < 0 ? "negative" : ""}>{formatPct(change, true)}</span></div></div><StateBadge state={data.selected.state} /></div><div className="chart-frame"><RegimeChart candles={data.candles} selected={data.selected} />{loading && <div className="chart-refresh">Refreshing completed candles…</div>}</div><div className="chart-legend">{data.selected.overlays.map(line => <span key={line.name}><i style={{ background: line.color }} />{line.name}</span>)}<span><i className="flip-dot" />Confirmed flip</span><span className="method-note">States effective next open</span></div></article>
      ...
      <section className="research-grid"><article className="research-card wide"><div className="section-heading"><div><p className="eyebrow">NEXT-OPEN BACKTEST</p><h2>Fixed presets, honest execution</h2></div><span className="assumption-pill">15 bps turnover</span></div><div className="backtest-table"><div className="backtest-head"><span>Model</span><span>CAGR</span><span>Max DD</span><span>Calmar</span><span>Exposure</span><span>Flips</span></div>{data.backtests.slice(0, 8).map(row => <div className="backtest-row" key={row.indicatorId}><strong>{row.displayName}</strong><span>{formatPct(row.cagr)}</span><span className="negative">{formatPct(row.maxDrawdown)}</span><b>{row.calmar?.toFixed(2) ?? "—"}</b><span>{formatPct(row.exposure)}</span><span>{row.flips}</span></div>)}</div></article>
        <article className="research-card"><p className="eyebrow">PUBLISHED BASELINE</p><h2>Faber 10-month rule</h2><StateBadge state={data.research.faber10Month.state === "unavailable" ? null : data.research.faber10Month.state} /><dl><div><dt>Monthly close</dt><dd>{formatPrice(data.research.faber10Month.close)}</dd></div><div><dt>10-month SMA</dt><dd>{formatPrice(data.research.faber10Month.sma10)}</dd></div></dl><p>Included in reports, not the daily/weekly flip matrix.</p></article>
        <article className="research-card"><p className="eyebrow">DATA PROVENANCE</p><h2>{data.dataset.sourceLabel} · {data.dataset.market}</h2><dl><div><dt>Calculation history</dt><dd>{data.dataset.candleCount.toLocaleString()} bars</dd></div><div><dt>Visible chart</dt><dd>Last {data.dataset.chartCandleCount} bars</dd></div><div><dt>Series begins</dt><dd>{formatDate(data.dataset.firstCandle)}</dd></div><div><dt>Storage</dt><dd>{data.dataset.storage === "sqlite" ? "Local SQLite cache" : data.dataset.storage === "provider" ? "Fresh provider import" : "Demo only"}</dd></div><div><dt>SHA-256</dt><dd className="checksum">{data.dataset.checksum.slice(0, 12)}…</dd></div><div><dt>Quality</dt><dd>{Object.values(data.dataset.quality).every(value => value === 0) ? "Passed" : "Review"}</dd></div></dl><p>Venues are never spliced and missing prices are never forward-filled.</p></article></section>
      <footer><p>BTC Regime Lab separates price regimes, confirmation, exits, and valuation. It does not optimize presets against Bitcoin history.</p><nav><a href="/api/v1/registry">Registry JSON</a><a href="/api/v1/series?source=bitstamp&timeframe=1w">Series JSON</a><a href="/api/v1/health">Source health</a></nav></footer>
    </>}
  </main>;
}
