...
type SpotQuote = { asset: AssetId; source: string; sourceLabel: string; market: string; denomination: string; price: number; retrievedAt: string };
type MatrixItem = {
...
type Payload = {
  ...
  dataset: { asset: AssetId; assetLabel: string; source: string; sourceLabel: string; market: string; denomination: string; timeframe: Timeframe; retrievedAt: string; checksum: string; stale: boolean; demo: boolean; storage: "provider" | "sqlite" | "demo"; warning: string | null; quality: { gaps: number; duplicates: number; malformed: number }; firstCandle: number | null; lastCandle: number | null; candleCount: number; chartCandleCount: number };
  candles: Candle[];
...
export default function RegimeDashboard() {
  ...
  const triggerCard = (label: string, value: number | null, variant: string) => <div className={`trigger ${variant}`}><span>{label}</span><strong>{value == null ? "Conditional" : formatPrice(value, denomination)}</strong><small>{data?.selected.thresholdKind} · as of {formatDate(data?.dataset.lastCandle, true)} UTC</small></div>;
