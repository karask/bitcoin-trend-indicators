  ...
  useEffect(() => {
    ...
    fetch(`/api/v1/dashboard?asset=${asset}&source=${source}&timeframe=${timeframe}&indicator=${indicator}${guppyQuery}`, { signal: controller.signal, cache: "no-store" }).then(async response => {
      ...
      const payload = "calculation" in result && result.calculation === "browser"
        ? { ...buildDashboardPayload(asset, source as "bitstamp" | "binance" | "kraken" | "coinbase", timeframe, indicator, result.daily, result.weekly, { superGuppy: guppyConfig }), ...(result.sources ? { sources: result.sources } : {}) } as Payload
        : result as Payload;
  ...
  return <main className="app-shell">
    ...
    {loading && !data ? <LoadingView /> : data && <>
      ...
      <section className="research-grid"><article className="research-card wide"><div className="section-heading"><div><p className="eyebrow">NEXT-OPEN BACKTEST</p><h2>Fixed presets, honest execution</h2></div><span className="assumption-pill">15 bps turnover</span></div><div className="backtest-table"><div className="backtest-head"><span>Model</span><span>CAGR</span><span>Max DD</span><span>Calmar</span><span>Exposure</span><span>Flips</span></div>{data.backtests.slice(0, 8).map(row => <div className="backtest-row" key={row.indicatorId}><strong>{row.displayName}</strong><span>{formatPct(row.cagr)}</span><span className="negative">{formatPct(row.maxDrawdown)}</span><b>{row.calmar?.toFixed(2) ?? "—"}</b><span>{formatPct(row.exposure)}</span><span>{row.flips}</span></div>)}</div></article>
        <article className="research-card"><p className="eyebrow">PUBLISHED BASELINE</p><h2>Faber 10-month rule</h2><StateBadge state={data.research.faber10Month.state === "unavailable" ? null : data.research.faber10Month.state} /><dl><div><dt>Monthly close</dt><dd>{formatPrice(data.research.faber10Month.close, data.dataset.denomination)}</dd></div><div><dt>10-month SMA</dt><dd>{formatPrice(data.research.faber10Month.sma10, data.dataset.denomination)}</dd></div></dl><p>Invest after a completed month closes at or above its 10-month SMA; move to cash below it, effective the following month. It has no neutral state and is reported outside the daily/weekly flip matrix.</p></article>
